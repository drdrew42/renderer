...
our @EXPORT = ();
our @EXPORT_OK =
  qw(build_library_directory_tree build_library_subject_tree build_library_textbook_tree);
...
sub buildTree {
    ...
    $branch->{name} = $name;
    my @dirs =
      File::Find::Rule->maxdepth(1)->relative(1)->directory->in($absoluteDir);
    if ( scalar(@dirs) == 0 ) {
        return undef;
    }
    ...
    for my $dir (@dirs) {
        ...
        if ($theBranch) {
            my @files = File::Find::Rule->file()->name("*.pg")
              ->in( $absoluteDir . "/" . $dir );
...
sub build_library_subject_tree {
    ...
    my $selectClause =
"select subj.name, ch.name, sect.name, path.path,pg.filename from $tables{dbsection} AS sect "
    ...
    my $results = $dbh->selectall_arrayref(
        "select subj.name from $tables{dbsubject} AS subj");

    my @subject_names = map { $_->[0] } @{$results};

    my $i = 0;    # counter to print to the screen.

    print "Building the subject-tree.  There are "
      . scalar(@subject_names)
      . " subjects\n";

    my @subject_tree
      ;           # array to store the individual library tree for each subject

    for my $subj_name (@subject_names) {

        my $subj = $subj_name;
        $subj =~ s/'/''/g; # update for postgres

        my $results = $dbh->selectall_arrayref(
"select ch.name from $tables{dbsubject} AS subj JOIN $tables{dbchapter} AS ch "
              . " ON subj.DBsubject_id = ch.DBsubject_id WHERE subj.name='$subj';"
        );
        my @chapter_names = map { $_->[0] } @{$results};

        my @chapter_tree
          ;    # array to store the individual library tree for each chapter
        ...
        for my $ch_name (@chapter_names) {
            ...
            my $ch = $ch_name;
            $ch =~ s/'/''/g;# update for postgres

            my $results = $dbh->selectall_arrayref(
                    "SELECT sect.name from $tables{dbsubject} AS subj "
            ...
            for my $sect_name (@section_names) {
                ...
                $sect =~ s/'/''/g;# update for postgres

                my $whereClause =
"WHERE sect.name='$sect' AND ch.name='$ch' AND subj.name='$subj'";
                ...
                my $clone = { %{$section_tree}
                };   # need to clone it before pushing into the @subfield array.
            ...
            my $clone = { %{$chapter_tree}
            };   # need to clone it before pushing into the @chapter_tree array.
    ...
    # you can check for errors (e.g., if after opening the disk gets full)
    print {$OUTFILE} to_json( \@subject_tree, { pretty => 1 } )
...
sub build_library_textbook_tree {
    ...
    my $results = $dbh->selectall_arrayref(
        "select * from $tables{textbook} ORDER BY title;");
    ...
    for my $textbook (@textbooks) {
        ...
        my @chapters =
          map { { chapter_id => $_->[0], name => $_->[1], number => $_->[2] } }
          @{$results};

        for my $chapter (@chapters) {

            my $results = $dbh->selectall_arrayref(
                    "select sect.section_id,sect.name,sect.number "
            ...
            my @sections = map {
                { section_id => $_->[0], name => $_->[1], number => $_->[2] }
            } @{$results};
        ...
        my $whereClause =
          "WHERE text.textbook_id='" . $textbook->{textbook_id} . "'";
    ...
    # you can check for errors (e.g., if after opening the disk gets full)
    print {$OUTFILE} to_json( \@textbooks, { pretty => 1 } )
      or die "Cannot write to $file";

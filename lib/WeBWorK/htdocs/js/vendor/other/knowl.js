...
var complete = new Event('knowlChanged');

function knowl_click_handler($el) {
  var evt = new Event('resizeme')
  ...
  if ($output_id.length > 0) {
     $("#kuid-"+uid).slideToggle("fast", ()=>{window.parent.postMessage('resizeme', '*')});
...
//    }
    ...
    if ($el.attr("class") == 'internal') {
      ...
      if(window.MathJax == undefined) {
            $knowl.slideDown("slow", ()=>{window.parent.postMessage('resizeme', '*')});
      }  else {
            MathJax.Hub.Queue(['Typeset', MathJax.Hub, $output.get(0)]);
            MathJax.Hub.Queue([ function() { $knowl.slideDown("slow", ()=>{window.parent.postMessage('resizeme', '*')}); }]);
      }
    } else {

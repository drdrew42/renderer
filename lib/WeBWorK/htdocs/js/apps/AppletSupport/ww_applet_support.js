...
function setHTMLAppletState(appletName, newState) {
	...
	const stateInput = ww_applet_list[appletName].stateInput;
...
class ww_applet {
	...
	setState(state) {
		...
		if (state.match(/^<xml>restart_applet<\/xml>/) ||
			...
			if (this.initialState.match(/^<xml>\s*<\/xml>/) || this.initialState.match(/^\s*$/)) {
				...
				// restart_applet.
				setHTMLAppletState(this.appletName, "<xml></xml>");
				...
				setHTMLAppletState(this.appletName, this.initialState);
	...
	submitAction() {
		...
		if (ww_preserve_applet_state.value.match(/^<xml>restart_applet<\/xml>/)) {
			...
			setHTMLAppletState(this.appletName);
			return;
		}
...
(() => {
	...
	function initializeAppletSupport() {
		// This portion of the code should only be executed once.
		if (typeof initializeAppletSupport.hasRun == 'undefined') {
			initializeAppletSupport.hasRun = true;

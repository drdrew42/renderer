...
sub alarm_handler {
    my $msg =
        "Timeout after processing this problem for "
      . TIMEOUT
...
# install a local warn handler to collect warnings  FIXME -- figure out what I meant to do here.
        ...
        || $translationOptions->{displayMode} eq "opaque_image" )
    {
        ...
        $image_generator = WeBWorK::PG::ImageGenerator->new(
            ...
            useMarkers => (
                     $imagesModeOptions{dvipng_align}
                  && $imagesModeOptions{dvipng_align} eq 'mysql'
            ),
...
# 2021 -- for the standaloneRenderer, PG.pl is pre-cached in Translator.pm
  ...
  # the source for the problem is already given to us as a reference to a string
        ...
        $sourceFilePath = $ce->{courseDirs}->{templates} . "/" . $sourceFilePath
          unless ( $sourceFilePath =~ /^\// );
...
EOF
    ...
    if ( $translationOptions->{processAnswers} ) {
     ...
     #warn "PG: retrieving the problem state and giving it to the translator\n";
        ...
        ( $result, $state ) = $translator->grade_problem(
            ...
            %{$formFields}
            ,    #FIXME?  this is used by sequentialGrader is there a better way
        );
    ...
    if ($image_generator) {
        my $sourceFile =
          $ce->{courseDirs}->{templates} . "/" . $problem->source_file;
        my %mtimeOption =

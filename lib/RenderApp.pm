...
sub startup {
    my $self       = shift;
    my $staticPath = $WeBWorK::Constants::WEBWORK_DIRECTORY
      . "/htdocs/";    #curfile->dirname->sibling('public')->to_string.'/';
    ...
    # Models
    $self->helper(
        newProblem => sub { shift; RenderApp::Model::Problem->new(@_) } );

    # helper to expose request data
    $self->helper(
        requestData2JSON => sub {
            my $c               = shift;
            my $hash            = {};

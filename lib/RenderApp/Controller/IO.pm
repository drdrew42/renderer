...
our $regex = {
	anyPg      => qr/.+\.p[gl]$/,
	allPathsPg =>
		qr/^(:?private\/|Contrib\/|webwork-open-problem-library\/Contrib\/|Library\/|webwork-open-problem-library\/OpenProblemLibrary\/)(?!\.\.\/).*\.p[gl]$/,
	publicOnlyPg =>
		qr/^(:?Contrib\/|webwork-open-problem-library\/Contrib\/|Library\/|webwork-open-problem-library\/OpenProblemLibrary\/)(?!\.\.\/).*\.pg$/,
	privateOnlyPg => qr/^private\/(?!\.\.\/).*\.p[gl]$/,
	allPaths      =>
		qr/^(:?private\/|Contrib\/|webwork-open-problem-library\/Contrib\/|Library\/|webwork-open-problem-library\/OpenProblemLibrary\/)(?!\.\.\/)/,
	publicOnly =>
		qr/^(:?Contrib\/|webwork-open-problem-library\/Contrib\/|Library\/|webwork-open-problem-library\/OpenProblemLibrary\/)(?!\.\.\/)/,
...
async sub writer {
	...
	push @$required, { field => 'problemSource', };
	my $validatedInput = $c->validateRequest({ required => $required });
	return unless $validatedInput;
...
async sub search {
	...
	my $sources =
		[ 'private/', 'webwork-open-problem-library/OpenProblemLibrary/', 'webwork-open-problem-library/Contrib/' ];
	$c->render_later;
...
async sub findUniqueSeeds {
	...
	if (scalar @returnKeys >= $numberOfSeeds) {
		return $c->render(json => { uniqueKeys => \@returnKeys });
	} else {
		return $c->render(
			json => {
				statusCode => 404,
...
async sub setTags {
	...
	$incomingTags->{DESCRIPTION} = [ $incomingTags->{DESCRIPTION} ]
		unless (ref($incomingTags->{DESCRIPTION}) =~ /ARRAY/);

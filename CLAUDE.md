...
| `LOCK_ON_PERFECT` | `1` | Legacy-lane lock policy: when truthy, a non-instructor scoring 100% triggers `isLocked=1` on the session (no further answerJWTs). Set `0` to keep sessions open even after a perfect score (e.g. for practice deployments where students may want to retry). |
| `LOCK_ON_SHOW_ANSWERS` | `1` | Legacy-lane lock policy: when truthy, a non-instructor request for `showCorrectAnswers` triggers `isLocked=1`. Set `0` to make reveal *ephemeral* — answers are shown for the current request but neither the lock nor the `showCorrectAnswers` flag persist into session state, allowing the LMS to reveal answers without permanently terminating the attempt. |

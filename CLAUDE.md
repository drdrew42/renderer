...
| `LOCK_ON_PERFECT` | `1` | Legacy-lane *hard*-ratchet policy: when truthy, a non-instructor scoring 100% triggers `isLocked=1` on the session (no further answerJWTs, no session updates). Set `0` to keep sessions open even after a perfect score (e.g. for practice deployments where students may want to retry). |
| `LOCK_ON_SHOW_ANSWERS` | `1` | Legacy-lane *hard*-ratchet policy: when truthy, a non-instructor request for `showCorrectAnswers` triggers `isLocked=1`. Set `0` to keep the session writable after reveal — the *soft* ratchet (`answersRevealed=1` in session state, surfaced to the answerURL recipient) still fires regardless. The LMS sees the reveal signal in the answerJWT and decides whether the session can be reused for further interaction. |

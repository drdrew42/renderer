...
subtest 'POST /render-api/solution returns solution body in message' => sub {
	$t->post_ok(
		'/render-api/solution' => form => {
			problemJWT    => $jwt_solution,
			problemSource => $pg_with_both,
			problemSeed   => 1234,
		}
	)->status_is(200)->json_is('/status' => 200);
...
subtest 'POST /render-api/solution: no solution block → empty message' => sub {
	...
	)->status_is(200)->json_is('/status' => 200)->json_is('/message' => '');
};
...
subtest 'POST /render-api/hint returns hint body in message' => sub {
	$t->post_ok(
		'/render-api/hint' => form => {
			problemJWT    => $jwt_hint,
			problemSource => $pg_with_both,
			problemSeed   => 1234,
		}
	)->status_is(200)->json_is('/status' => 200);
	like($t->tx->res->json->{message}, qr/small even number/, 'message contains expected hint content');
};
...
subtest 'solution endpoint mints no JWTs' => sub {
	...
	is_deeply(\@keys, [ 'message', 'status' ], 'response carries only status+message — no JWTs, no extras');
};
...
subtest 'typ at top level, problemSource/seed inside webwork envelope' => sub {
	...
	$t->post_ok('/render-api/solution' => form => { problemJWT => $jwt })->status_is(200);
	like($t->tx->res->json->{message}, qr/42/, 'source+seed from webwork claims rendered without form params');
};
...
subtest 'claims win over form params for source+seed' => sub {
	...
	like($t->tx->res->json->{message}, qr/42/, 'rendered the claim-bound source, not the form-data override');
};
...
subtest 'sourceFilePath claim resolves via content cache' => sub {
	...
	$t->post_ok('/render-api/solution' => form => { problemJWT => $jwt })->status_is(200);
	like($t->tx->res->json->{message}, qr/42/, 'solution resolved from sourceFilePath claim via content cache');
};
...
subtest 'unresolvable sourceFilePath → 404' => sub {
	...
	$t->post_ok('/render-api/solution' => form => { problemJWT => $jwt })->status_is(404);
};
...
PG
	...
	$t->post_ok('/render-api/solution' => form => { problemJWT => $jwt })->status_is(200);
	like($t->tx->res->json->{message},
		qr/INJECTED_OK/, 'cache-only macro loaded via injectedMacros — sentinel rendered into solution');
};
...
PG
	...
	)->status_is(500)->json_is('/status' => 500);
...
subtest 'error responses follow { status, message } shape' => sub {
	...
	)->status_is(401)->json_is('/status' => 401);
	...
	)->status_is(400)->json_is('/status' => 400);

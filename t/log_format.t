...
subtest 'leading hashref merges to top level, no nested message' => sub {
	my $e = decode_json($fmt->(1_716_000_000, 'info', { type => 'request', status => 200, duration_ms => '12.3' }));
...
subtest 'envelope fields are not clobbered by the hashref' => sub {
	my $e =
		decode_json($fmt->(1_716_000_000, 'info', { service => 'spoofed', level => 'spoofed', type => 'request' }));
